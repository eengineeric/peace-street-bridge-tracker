-- Peace Street Bridge Tracker automatic incident migration.
...
create index if not exists bridge_incidents_incident_at_idx
  on public.bridge_incidents (incident_at desc);

alter table public.bridge_incidents enable row level security;

alter table public.bridge_reports add column if not exists incident_id uuid references public.bridge_incidents(id) on delete set null;
alter table public.bridge_reports add column if not exists detected_incident_at timestamptz;
alter table public.bridge_reports add column if not exists confidence double precision;
alter table public.bridge_reports add column if not exists extraction_method text;
alter table public.bridge_reports add column if not exists excerpt text;

alter table public.bridge_reports drop constraint if exists bridge_reports_status_check;
alter table public.bridge_reports add constraint bridge_reports_status_check
  check (status in ('candidate', 'confirmed', 'rejected', 'auto_confirmed', 'duplicate', 'skipped'));
...
begin
  -- Serialize incident matching so simultaneous cron/manual scans cannot create duplicates.
  ...
  -- Articles describing an event within 30 minutes of an existing event are sources for
  -- that same incident, not another incident.
  select * into v_incident
  from public.bridge_incidents
  where incident_at between p_detected_incident_at - interval '30 minutes'
                        and p_detected_incident_at + interval '30 minutes'
  order by abs(extract(epoch from (incident_at - p_detected_incident_at)))
  limit 1;

  if not found then
    insert into public.bridge_incidents (incident_at, title, confidence)
    values (p_detected_incident_at, p_title, p_confidence)
    returning * into v_incident;
    v_created := true;
  else
    update public.bridge_incidents
    set confidence = greatest(confidence, p_confidence), updated_at = now()
  ...
  insert into public.bridge_reports (
    ...
    p_published_at, (p_detected_incident_at at time zone 'America/New_York')::date,
